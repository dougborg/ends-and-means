...
const expectedKinds = {
  "zapatista-autonomy-indigenous-context": "classification",
  "zapatista-european-theory-boundary": "editorial-interpretation",
  "jbg-formation-declaration": "attributed-proposal",
  "jbg-formal-delegation": "attributed-proposal",
  "jbg-formal-regional-functions": "attributed-proposal",
  "jbg-formal-municipal-functions": "attributed-proposal",
  "jbg-declared-ezln-oversight": "attributed-proposal",
  "jbg-rotation-rules-in-use": "observation",
  "jbg-rotation-learning-purpose": "observation",
  "jbg-reporting-practice": "observation",
  "jbg-accounting-practice": "observation",
  "jbg-gender-participation-limit": "observation",
  "jbg-external-project-control": "observation",
  "jbg-civil-military-authority-limit": "observation",
  "zapatista-hybrid-authority-interpretation": "classification",
  "zapatista-reach-limit": "observation",
  "zapatista-2023-reorganization-declaration": "attributed-proposal",
  "zapatista-2023-caracoles-continuity": "attributed-proposal",
  "zapatista-2023-practice-open": "observation",
  "zapatista-anarchist-resemblance": "classification",
  "zapatista-anarchism-boundary": "editorial-interpretation",
  "zapatista-accountability-assessment": "editorial-interpretation",
} as const;
const expectedTexts = [
...
describe("Zapatista caracol and Good Government Council case", () => {
  it("publishes exact bounded Case, episode, Event, Transition, and freshness tuples", () => {
    expect(entityById("zapatista-autonomy-chiapas-1994-present")).toMatchObject(
      {
        kind: "case",
        episodeIds: [
    ...
    expect(entityById("zapatista-caracol-jbg-episode-2003-2023")).toMatchObject(
      {
        kind: "case-episode",
        startDate: { year: 2003, month: 8, certainty: "approximate" },
    ...
    expect(
      entityById("zapatista-caracoles-jbg-formation-announced-2003"),
    ).toMatchObject({
      kind: "event",
      startDate: { year: 2003, month: 7, day: 21, certainty: "exact" },
    });
    expect(entityById("zapatista-autonomy-reorganization-2023")).toMatchObject({
      kind: "event",
    ...
    expect(entityById("zapatista-jbg-to-gal-transition-2023")).toMatchObject({
      kind: "transition",
  ...
  it("keeps organizations distinct and anarchism subordinate to Indigenous autonomy", () => {
    for (const id of [
      ...
      expect(entityById(id)?.kind).toBe("organization");
    const relations = relationshipsFrom(
      "zapatista-caracol-jbg-episode-2003-2023",
    );
    expect(
      relations.filter(({ object }) => object.id === "indigenous-autonomy"),
    ).toHaveLength(1);
    expect(relations.filter(({ object }) => object.id === "anarchism")).toEqual(
      [
...
describe("Zapatista evidence ledger", () => {
  it("has at least ten atomic located Statements and at least five authoritative Sources", () => {
    ...
    expect(
      statementIds.every(
        (id) =>
          citationsFor(id).length > 0 &&
          citationsFor(id).every(({ locator }) => locator.trim()),
      ),
    ).toBe(true);
    expect(
      new Set(
  ...
  it("pins every intended Statement kind and every evidence tuple", () => {
    ...
    expect(
      statementIds.map((id) => {
        const statement = entityById(id);
        return statement?.kind === "statement" ? statement.text : undefined;
      }),
    ).toEqual(expectedTexts);
    const actual = statementIds.flatMap((id) =>
      citationsFor(id).map(({ object, role, locator }) => [
        id,
        object.id,
        role,
        locator,
    ...
    const mutation = expectedCitationTuples.map((row, index) =>
      index === 0 ? [row[0], row[1], row[2], "mutated locator"] : [...row],
    );
    expect(actual).not.toEqual(mutation);
    expect(entityById("gunderson-autonomist-critique-source")).toMatchObject({
      contributorDisplay: ["Christopher Gunderson"],
      identifiers: { doi: "10.1521/siso.2018.82.4.531" },
      publisher: "SAGE Publications",
      resourceLinks: [
        {
          purpose: "publisher",
          url: "https://doi.org/10.1521/siso.2018.82.4.531",
        },
      ],
      description: expect.stringContaining("pp. 531–554"),
    });
    expect(entityById("speed-exercising-rights-work")).toMatchObject({
      workType: "other",
      originalPublicationYear: 2007,
    });
    expect(entityById("speed-exercising-rights-source")).toMatchObject({
      contributorDisplay: ["Shannon Speed"],
      identifiers: { doi: "10.1017/CBO9780511819193.007" },
      publisher: "Cambridge University Press",
      description: expect.stringContaining("chapter 4, pp. 163–192"),
    });
  });
});

describe("Zapatista public narrative", () => {
  it("exposes challenge, criterion, research obligations, and canonical narrative without placeholders", () => {
    ...
    expect(entityById("zapatista-participation-and-inclusion")?.kind).toBe(
      "challenge",
    );
    expect(entityById("affected-community-accountability")).toMatchObject({
      kind: "criterion",
      normativeAssumptions: expect.arrayContaining([
        expect.stringContaining("subject to governing decisions"),
      ]),
    });
    expect(
      researchObligationsForTarget(
      ...
      "zapatista-participation-gender-authority",
      "zapatista-post-2023-rules-in-use",
    ]);
    const dossier = dossierForSubject(
      "case",
      "zapatista-autonomy-chiapas-1994-present",
    );
    expect(dossier?.description).toBe(
      "In 2003 the EZLN announced caracoles seating regional Good Government Councils composed of delegates from autonomous municipalities. Participant accounts described rotation, reporting, receipt-based accounting, and uneven participation; Andrews found that councils set terms for outside aid work, while Gunderson argued that EZLN political-military authority continued. In 2023 the EZLN declared a shift to community-level Local Autonomous Governments, but their rules in use and outcomes remain unverified.",
    );
    ...
    expect(
      dossier?.sections.every(
        ({ body }) => body && !/placeholder|legacy|migration/i.test(body),
      ),
    ).toBe(true);
    expect(entitiesOfKind("case").map(({ id }) => id)).toContain(
      "zapatista-autonomy-chiapas-1994-present",
    );
  });
});
