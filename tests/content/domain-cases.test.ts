...
const base = {
  description: "A scoped case-model test record.",
...
const documents: AuthoringDocument[] = [
  ...
  entity({
    ...
    startDate: {
      ...
      note: "Fixture boundary used to exercise uncertainty.",
    },
  ...
  entity({
    id: "example-change-kind",
    kind: "concept",
    label: "Synthetic institutional change",
    schemeIds: [],
    scopeNote: "Synthetic Event-kind fixture only.",
    ...base,
  }),
  entity({
    id: "example-material-source",
    kind: "source",
    label: "Synthetic material-change source",
    title: "Synthetic source",
    sourceType: "other",
    workId: "example-material-work",
    publicationStatus: "reviewed",
    description: "A synthetic source used only for a model-boundary test.",
  }),
  entity({
    id: "example-material-change",
    kind: "event",
    label: "Synthetic material change",
    description:
      "A synthetic reviewed Event used only for model-boundary tests.",
    eventKindIds: ["example-change-kind"],
    placeIds: ["example-region"],
    startDate: { year: 2026, certainty: "exact" },
    descriptionStatementIds: ["example-outcome"],
    publicationStatus: "reviewed",
  }),
  {
    documentType: "relationships",
    subject: { kind: "statement", id: "example-outcome" },
    relationships: [
      {
        id: "example-outcome-cites-material-source",
        predicate: "cites",
        subject: { kind: "statement", id: "example-outcome" },
        object: { kind: "source", id: "example-material-source" },
        role: "supports",
  ...
  entity({
    id: "example-material-work",
    kind: "work",
    label: "Synthetic material-change work",
    title: "Synthetic source",
    workType: "other",
    publicationStatus: "reviewed",
    description: "A synthetic work used only for a model-boundary test.",
  }),
];
...
describe("bounded Case model", () => {
  it("compiles an ongoing Case with a bounded episode and separated observations", () => {
    ...
    expect(graph.indexes.entitiesById["example-ongoing-case"]?.kind).toBe(
      "case",
    );
    expect(graph.indexes.entitiesById["example-case-episode"]?.kind).toBe(
      "case-episode",
    );
  });
  ...
  it("requires freshness metadata for ongoing Cases", () => {
    ...
    if (
      ongoingCase?.documentType === "entity" &&
      ongoingCase.entity.kind === "case"
    ) {
    ...
    expect(errors).toContain(
      "example-ongoing-case: asOf requires an ISO calendar date",
    );
    expect(errors).toContain(
      "example-ongoing-case: lastReviewedAt requires an ISO calendar date",
    );
    expect(errors).toContain(
      "example-ongoing-case: ongoing Case requires freshness",
    );
  });
  ...
  it("rejects unresolved Places, Statements, episodes, and parent Cases", () => {
    ...
    if (
      ...
      episode.entity.caseId = "missing-case";

    const errors = validateAuthoringDocuments(invalid);
    expect(errors).toContain(
      "example-ongoing-case: unresolved Place missing-place",
    );
    expect(errors).toContain(
      "example-ongoing-case: unresolved Statement missing-statement",
    );
    expect(errors).toContain(
      "example-ongoing-case: unresolved or mismatched Case Episode missing-episode",
    );
    expect(errors).toContain(
      "example-case-episode: unresolved parent Case missing-case",
    );
  });
});

describe("bounded Case date validation", () => {
  ...
  it("rejects inverted date ranges", () => {
    ...
    expect(validateAuthoringDocuments(invalid)).toContain(
      "example-case-episode: startDate must not be after endDate",
    );
  });
  ...
  it("rejects impossible calendar dates", () => {
    ...
    if (
      ...
      ongoingCase.entity.asOf = "2026-02-30";
    if (
      episode?.documentType === "entity" &&
      episode.entity.kind === "case-episode"
    ) {
      episode.entity.startDate = {
        year: 2025,
        month: 2,
        day: 30,
        certainty: "exact",
    ...
    expect(errors).toContain(
      "example-case-episode: startDate day is invalid for its month",
    );
  });
});

describe("bounded Case reciprocal references", () => {
...
describe("material-change Event freshness metadata", () => {
  it("validates material-change pointers as cited, deterministic freshness metadata", () => {
    ...
    if (
      ...
      ongoingCase.entity.materialChangeEventIds = [
        "missing-event",
        "missing-event",
      ];
    expect(validateAuthoringDocuments(invalid)).toEqual(
...
describe("material-change Event scope boundaries", () => {
  it("rejects every material-change boundary violation", () => {
    const variants: Array<[string, (docs: AuthoringDocument[]) => void]> = [
      [
        "material-change Event IDs must be sorted",
        (docs) => {
          const value = docs[5];
          if (value?.documentType === "entity" && value.entity.kind === "case")
            value.entity.materialChangeEventIds = [
              "z-event",
      ...
      [
        "must be reviewed or published",
        (docs) => {
          const value = docs[9];
          if (value?.documentType === "entity")
            value.entity.publicationStatus = "in-review";
        },
      ],
      [
        "falls outside the Case review period",
        (docs) => {
          const value = docs[9];
          if (value?.documentType === "entity" && value.entity.kind === "event")
            value.entity.startDate = { year: 1993, certainty: "exact" };
        },
      ],
      [
        "falls outside the Case review period",
        (docs) => {
          const value = docs[9];
          if (value?.documentType === "entity" && value.entity.kind === "event")
            value.entity.startDate = { year: 2027, certainty: "exact" };
        },
      ],
      [
        "must share a Place with the Case",
        (docs) => {
          docs.push(
            entity({
              id: "unrelated-region",
              kind: "place",
              label: "Unrelated region",
              placeType: "region",
              ...base,
            }),
          );
          const value = docs[9];
          if (value?.documentType === "entity" && value.entity.kind === "event")
            value.entity.placeIds = ["unrelated-region"];
        },
      ],
      [
        "unresolved material-change Event",
        (docs) => {
          const value = docs[5];
          if (value?.documentType === "entity" && value.entity.kind === "case")
            value.entity.materialChangeEventIds = ["example-material-source"];
        },
      ],
    ];
    for (const [message, mutate] of variants) {
      ...
      mutate(invalid);
      expect(
        validateAuthoringDocuments(invalid).some((error) =>
...
describe("material-change Event evidence", () => {
  it("rejects every material-change evidence violation", () => {
    const variants: Array<[string, (docs: AuthoringDocument[]) => void]> = [
      [
        "requires an Event-owned reviewed description Statement",
        (docs) => {
          docs.splice(10, 1);
        },
      ],
      [
        ...
        (docs) => {
          const value = docs[8];
          if (value?.documentType === "entity")
            value.entity.publicationStatus = "research-needed";
        },
      ],
      [
        "requires an Event-owned reviewed description Statement",
        (docs) => {
          const relation = docs[10];
          if (
            relation?.documentType === "relationships" &&
            relation.relationships[0]?.predicate === "cites"
          )
            relation.relationships[0].object.id = "missing-source";
        },
      ],
      [
        "requires an Event-owned reviewed description Statement",
...
describe("material-change Event publication boundaries", () => {
  ...
  it("rejects freshness pointers on ended Cases and compiles deterministically across input permutations", () => {
    ...
    const value = ended[5];
    if (value?.documentType === "entity" && value.entity.kind === "case")
      value.entity.endDate = { year: 2026, certainty: "exact" };
    expect(validateAuthoringDocuments(ended)).toContain(
      "example-ongoing-case: ended Case must not declare material-change Event IDs",
    );
