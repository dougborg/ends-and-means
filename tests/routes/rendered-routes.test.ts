...
const routeFile = (route: string) => path.join(dist, route.replace(/^\//, ""), "index.html");
...
async function resolves(pathname: string): Promise<boolean> {
  const candidate = path.join(dist, decodeURIComponent(pathname).replace(/^\//, ""));
  for (const possibility of path.extname(candidate) ? [candidate] : [candidate, `${candidate}.html`, path.join(candidate, "index.html")]) {
    try { if ((await stat(possibility)).isFile()) return true; } catch { /* Try the next static-host form. */ }
  }
  return false;
}
...
describe("canonical public routes", () => {
  ...
  it("keeps archived research outside production imports", async () => {
    const productionFiles = (await walk(path.join(root, "src"))).filter((file) => /\.(?:astro|ts|css)$/.test(file));
    for (const file of productionFiles) {
      const source = await readFile(file, "utf8");
      expect(source, path.relative(root, file)).not.toMatch(/archive\/legacy-research|content\/framework|lib\/(?:framework|prototype|content)/);
    }
  });
  ...
  it("has no broken internal document or asset links", async () => {
    ...
    for (const file of (await walk(dist)).filter((candidate) => candidate.endsWith(".html"))) {
      for (const href of hrefs(await readFile(file, "utf8"))) {
        ...
        if (url.origin !== "https://endsandmeans.info" || href.startsWith("mailto:") || href.startsWith("tel:") || href.startsWith("#")) continue;
