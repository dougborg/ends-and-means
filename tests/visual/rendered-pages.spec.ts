...
async function expectMouseActivationNotCanceled(
  link: Locator,
  eventType: "auxclick" | "click",
  options: { button: number; ctrlKey?: boolean; metaKey?: boolean },
) {
  const eventResult = await link.evaluate(
    (element, { eventType, options }) => {
      let preventedBeforeGuard: boolean | undefined;
      const terminalGuard = (event: Event) => {
        preventedBeforeGuard = event.defaultPrevented;
        event.preventDefault();
      };
      // The application is already loaded before this terminal window listener
      // is installed. It observes target, document, and earlier window handlers,
      // then alone suppresses the synthetic event's browser-owned default.
      window.addEventListener(eventType, terminalGuard, { once: true });
      ...
      try {
        ...
        return {
          ...
          dispatchResult,
          preventedBeforeGuard,
          reachedTerminalGuard: preventedBeforeGuard !== undefined,
        };
      } finally {
        window.removeEventListener(eventType, terminalGuard);
      }
    },
    { eventType, options },
  );
  // Browser engines own tab creation and commit, which hosted headless cannot
  // attest reliably. The terminal guard leaves the real anchor unchanged and
  // prevents navigation only after recording whether application code canceled.
  expect(eventResult).toEqual({
    defaultPrevented: true,
    dispatchResult: false,
    preventedBeforeGuard: false,
    reachedTerminalGuard: true,
  });
}
...
test("mobile links preserve native pointer, touch, and keyboard navigation", async ({
  browser,
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
...
test("mobile links preserve uncanceled native modifier activation", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
  await page.goto("/research/");
  ...
  ).toEqual({ href: "/challenges/", tagName: "A", target: null });
  await expectMouseActivationNotCanceled(questions, "click", {
    button: 0,
  ...
  ).toEqual({ href: "/challenges/", target: null });

  await expect(page).toHaveURL(/\/research\/$/);
});

// Middle-click has its own auxclick cancellation path, so the modifier-click
// check cannot protect this independently breakable native affordance.
test("mobile links preserve uncanceled native middle-click activation", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
  await page.goto("/research/");
  ...
  ).toEqual({ href: "/reading/", tagName: "A", target: null });
  await expectMouseActivationNotCanceled(sources, "auxclick", { button: 1 });
  expect(
    await sources.evaluate((link) => ({
      href: link.getAttribute("href"),
      target: link.getAttribute("target"),
    })),
  ).toEqual({ href: "/reading/", target: null });

  await expect(page).toHaveURL(/\/research\/$/);
});

test("preventing modified click breaks native modifier activation", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
  await page.goto("/research/");
  ...
  await expect(
    expectMouseActivationNotCanceled(questions, "click", {
      button: 0,
      ...(process.platform === "darwin"
        ? { metaKey: true }
        : { ctrlKey: true }),
...
test("preventing auxclick breaks native middle-click navigation", async ({
  ...
  await expect(
    expectMouseActivationNotCanceled(sources, "auxclick", { button: 1 }),
  ).rejects.toThrow();

  await expect(page).toHaveURL(/\/research\/$/);
});

test("delegated link cancellation breaks native modifier activation", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 900 });
  await page.goto("/research/");
