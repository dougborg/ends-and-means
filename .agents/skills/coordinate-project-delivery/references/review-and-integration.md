...
Use focused checks while authoring and full verification at final handoff; renew affected evidence after later changes or rebases. Use [recorded executions](../../../../docs/execution-records.md) for supported command outcomes. Provide store/run identity at launch so the coordinator can consume terminal status without a second manual completion account; the author retains tool-handle polling and cleanup. Keep one concise candidate handoff with exact commit, evidence references, new findings or concrete blocker, next action, and owned cleanup. Record observable review rounds, waiting intervals, and environment failures privately; unknown historical intervals remain unknown. Do not fabricate an efficiency percentage.
