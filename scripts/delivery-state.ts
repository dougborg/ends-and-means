...
const ownershipSchema = z
  .object({ owner: z.string().min(1), branch: z.string().min(1), worktree: z.string().min(1) })
  .strict();
...
export const deliveryItemSchema = z
  .object({
    ...
    linkedPullRequestStates: z.array(z.enum(["OPEN", "CLOSED", "MERGED"])).optional(),
...
export const deliverySnapshotSchema = z
  .object({
    project: z.object({ number: z.number().int().positive(), title: z.string().min(1), public: z.boolean() }).strict(),
    capturedAt: dateTime,
...
function isReadyCandidate(item: DeliveryItem) {
  if (item.type !== "Issue" || item.state !== "OPEN" || item.status !== "Ready") return false;
  if (!item.priority || !item.workstream || !item.labels.includes("status:ready")) return false;
  if (!hasExecutableAcceptance(item.body) || hasUnmetDependency(item.body)) return false;
...
export function orderedReady(items: DeliveryItem[]) {
  return items
    ...
    .toSorted((left, right) => {
      const rank = (priorityRank.get(left.priority ?? "Later") ?? 2) - (priorityRank.get(right.priority ?? "Later") ?? 2);
...
function statusFindings(item: DeliveryItem): DeliveryFinding[] {
  ...
  if (item.state === "OPEN" && item.status === "Done") {
    findings.push({ code: "STATUS_DONE_OPEN", item: item.number, message: `#${item.number} is open but marked Done.` });
  }
...
function labelStatusFindings(item: DeliveryItem): DeliveryFinding[] {
  ...
  if (item.status !== "Blocked" && item.labels.includes("status:blocked")) {
    findings.push({ code: "STATUS_STALE_BLOCKED_LABEL", item: item.number, message: `#${item.number} retains status:blocked while ${item.status}.` });
  }
  return findings;
}
...
function activeEvidenceFindings(item: DeliveryItem): DeliveryFinding[] {
  ...
  if (["In progress", "In review"].includes(item.status) && item.historyLinear !== true) {
    findings.push({ code: "LINEAR_HISTORY", item: item.number, message: `#${item.number} is not proven rebase-only and linear.` });
  }
  if (item.status === "In review" && (!item.reviewEvidence?.copilot || !item.reviewEvidence.adversarial)) {
    findings.push({ code: "REVIEW_EVIDENCE", item: item.number, message: `#${item.number} lacks completed Copilot and adversarial review evidence.` });
  }
  return findings;
}
...
function auditReady(items: DeliveryItem[]): DeliveryFinding[] {
  ...
  for (const item of ready.filter((candidate) => !isReadyCandidate(candidate))) {
    findings.push({ code: "READY_NOT_EXECUTABLE", item: item.number, message: `#${item.number} is not dependency-free and independently executable.` });
  }
  return findings;
}
...
function auditWip(items: DeliveryItem[], capturedAt: string, staleDays: number): DeliveryFinding[] {
  ...
  if (active.length > 3) findings.push({ code: "WIP_LIMIT", message: `${active.length} implementation items are In progress; maximum is 3.` });
  for (const stream of workstreams) {
    const inStream = active.filter((item) => item.workstream === stream);
    if (inStream.length > 1) findings.push({ code: "WIP_DUPLICATE_STREAM", message: `${inStream.length} ${stream} items are In progress.` });
  }
  for (const item of active) {
    if (!item.workstream) findings.push({ code: "WIP_WORKSTREAM", item: item.number, message: `#${item.number} has no workstream.` });
  ...
  for (const item of active) {
    if (!item.updatedAt) findings.push({ code: "WIP_FRESHNESS_UNAVAILABLE", item: item.number, message: `#${item.number} has no updatedAt value; freshness is unavailable.` });
    else if (Date.parse(item.updatedAt) < staleCutoff) findings.push({ code: "WIP_STALE", item: item.number, message: `#${item.number} has been inactive for more than ${staleDays} days.` });
  }
  return findings;
}
...
function auditLearnerSequence(items: DeliveryItem[]): DeliveryFinding[] {
  ...
  const findings: DeliveryFinding[] = [];
  if (contract && contract.status === "In progress" && vision?.status !== "Done") findings.push({ code: "LEARNER_SEQUENCE", item: 120, message: "#120 started before #119 completed." });
  if (prototype && ["In progress", "In review", "Done"].includes(prototype.status) && contract?.status !== "Done") findings.push({ code: "LEARNER_SEQUENCE", item: 130, message: "#130 advanced before #120 completed." });
  for (const item of downstream.filter((candidate) => ["In progress", "In review", "Done"].includes(candidate.status))) {
    if (prototype?.status !== "Done") findings.push({ code: "LEARNER_SEQUENCE", item: item.number, message: `#${item.number} advanced before #130 completed.` });
  }
  return findings;
}
...
export function auditDeliverySnapshot(snapshot: DeliverySnapshot, staleDays = 14): DeliveryFinding[] {
  const findings: DeliveryFinding[] = [];
  if (snapshot.project.number !== 7 || snapshot.project.title !== "Ends and Means — Delivery") {
    findings.push({ code: "PROJECT_IDENTITY", message: `Expected Ends and Means — Delivery Project #7; received ${snapshot.project.title} #${snapshot.project.number}.` });
  }
  if (snapshot.project.public) findings.push({ code: "PROJECT_VISIBILITY", message: "Delivery Project is public; the harness never changes visibility." });
  for (const label of requiredTrackLabels) {
    if (!snapshot.repositoryLabels.includes(label)) findings.push({ code: "TRACK_LABEL", message: `Required track label ${label} is missing.` });
  }
  findings.push(...auditReady(snapshot.items), ...auditWip(snapshot.items, snapshot.capturedAt, staleDays), ...auditLearnerSequence(snapshot.items));
  for (const item of snapshot.items) {
    findings.push(...statusFindings(item), ...activeEvidenceFindings(item));
    if (item.status === "Blocked" && !hasConcreteBlocker(item.body)) findings.push({ code: "BLOCKER_UNNAMED", item: item.number, message: `#${item.number} is Blocked without a concrete named condition.` });
  }
  return findings;
}
